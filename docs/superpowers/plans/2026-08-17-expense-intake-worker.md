...
> **Post-implementation correction (2026-08-19):** end-to-end testing against the real Google API revealed that a bare (non-Workspace) service account cannot create new Drive/Sheets files at all (no storage quota of its own) — so the "auto-creates + shares each house's Google Sheet" plan below is **not what shipped**. What actually shipped: Sheet creation stays manual (create by hand, share with the service account as Editor — matching every earlier step's README), the script's `createHouseSheets` became `prepareHouseSheets` (writes the header row into an already-shared Sheet, no create/share calls), `buildOnboardingSql`/`onboardClient`/the CLI entrypoint/the D1-write automation are unchanged and accurate as described below. `getGoogleAccessToken`'s `scope` param and `DRIVE_FILE_SCOPE`/`createSpreadsheet`/`shareSpreadsheetWithEmail` (Tasks 45-46) were built, verified not to work, and removed rather than kept as dead code. The Windows `execFileSync('npx', ...)` call in Task 49 also needed `shell: true` added, discovered in the same testing pass (Windows won't spawn a `.cmd` shim directly without going through a shell). Verified working end-to-end against a real Sheet and local D1 afterward.

**Design spec:** `docs/superpowers/specs/2026-08-18-expense-intake-onboarding-cli-design.md` (approved by the project owner; see its own post-implementation correction note). This is the final Build Order step: a locally-run script that onboards a new client end-to-end — creates the `clients`/`houses`/`authorized_senders` D1 rows, and writes each house's Sheet header row — replacing the manual SQL every prior step's README has pointed at as a stopgap (Sheet creation itself remains manual, per the correction above).

**Interface (from the design spec, as corrected above):** `node expense-intake/scripts/onboard-client.js <config.json> <service-account.json> [--local]`. The pure/testable logic (`validateConfig`, `prepareHouseSheets`, `buildOnboardingSql`, `onboardClient`) lives in a new `src/onboarding.js`; the CLI script itself is a thin entrypoint that reads the two files, calls `onboardClient` with real dependencies (including a `runWrangler` that writes the generated SQL to a temp file and shells out to `wrangler d1 execute --file=`, with `shell: true` for Windows compatibility), and prints a summary.
