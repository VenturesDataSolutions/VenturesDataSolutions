...
> **Post-implementation correction (2026-08-19):** the "auto-creates + shares each house's Google Sheet" plan below turned out not to be possible. A bare (non-Workspace) Google Cloud service account has no Drive storage quota of its own and cannot create new files — confirmed against the real Sheets API (`spreadsheets.create` returns "The caller does not have permission"), discovered while testing this script end-to-end. What actually shipped: Sheet creation stays manual (the operator creates each house's Sheet by hand and shares it with the service account as Editor — the same model every earlier step already assumed), and the script's automation covers writing the header row into that already-shared Sheet plus the full D1 write. The `getGoogleAccessToken` `scope` parameter, `DRIVE_FILE_SCOPE`, `createSpreadsheet`, and `shareSpreadsheetWithEmail` described below were built, then removed once this was discovered — they're kept out of the current codebase entirely rather than left in as dead code. The rest of this document (D1 writes, invocation, validation, architecture split) is accurate as shipped.
